...
s.state,
    ...
    select * from city_performance_table where best_performance_rank <=10;
...
s.state,
    ...
    rank() over(order by  sum(
        ...
        ) asc) as lowest_performance_rank
    from
    orders o
    inner join order_details od on o.order_id = od.order_id
    inner join product_variants pv on pv.variant_id = od.variant_id
...
s.state,
    ...
    select * from city_performance_table where lowest_performance_rank <=10;